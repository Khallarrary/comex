...
public class ProdutoController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity cadastrar(@RequestBody @Valid DadosCadastroDTO dadosCadastroDTO, UriComponentsBuilder uriComponentsBuilder) {
        Fabricante fabricante = fabricanteRepository.findByNome(dadosCadastroDTO.nomeFabricante());
        if(fabricante == null) {
            fabricante = new Fabricante(dadosCadastroDTO);
            fabricanteRepository.save(fabricante);
        }
        var produto = new Produto(dadosCadastroDTO, fabricante);
        produtoRepository.save(produto);

        var uri = uriComponentsBuilder.path("{id}").buildAndExpand(produto.getId()).toUri();
    ...
    @PutMapping
    @Transactional
    public ResponseEntity atualizar(@RequestBody @Valid DadosAtualizarProdutoDTO produtoDTO){
